...
export const getFooter = catchAsyncErrors(async (req, res, next) => {
  ...
  const footer = await footerModel.findOne({ user: userId }).populate('user', 'fullName email');
...
export const createFooter = catchAsyncErrors(async (req, res, next) => {
  ...
  // Check if footer already exists for this user (whether regular user or admin)
  let query = {};
  if (req.user.role === 'admin') {
    query = { $or: [{ user: req.user.id }, { admin: req.user.id }] };
  } else {
    query = { user: req.user.id };
  }
  const existingFooter = await footerModel.findOne(query);
  ...
  if (existingFooter) {
    ...
    footerData.user = req.user.id;
    
    // If user is admin, store the same ID in admin field too
    if (req.user.role === 'admin') {
      footerData.admin = req.user.id;
    }
...
export const updateFooter = catchAsyncErrors(async (req, res, next) => {
  ...
  // Create query to find footer by user ID or admin ID (same entity, different roles)
  let query = {};
  ...
  let footer = await footerModel.findOne(query);
...
export const deleteFooter = catchAsyncErrors(async (req, res, next) => {
  ...
  const footer = await footerModel.findOne(query);
