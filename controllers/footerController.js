...
export const getFooter = catchAsyncErrors(async (req, res, next) => {
  ...
  const footer = await footerModel.findOne({ user: userId });
...
export const createFooter = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new ErrorHandler(`Error processing file upload: ${error.message}`, 500)
    );
  }

  // Extract all form fields from request body
  const { address, phone, description, socialIcons, recentPosts, dayTimes } =
    req.body;
  ...
  try {
    if (socialIcons) {
      parsedSocialIcons =
        typeof socialIcons === "string" ? JSON.parse(socialIcons) : socialIcons;
    }

    if (recentPosts) {
      parsedRecentPosts =
        typeof recentPosts === "string" ? JSON.parse(recentPosts) : recentPosts;
    }

    if (dayTimes) {
      parsedDayTimes =
        typeof dayTimes === "string" ? JSON.parse(dayTimes) : dayTimes;
    }
  } catch (error) {
    console.error("Error parsing JSON data:", error);
    return next(
      new ErrorHandler(`Error parsing form data: ${error.message}`, 400)
    );
  }
...
export const updateFooter = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    if (req.files) {
      ...
      if (req.files.footerImage && req.files.footerImage.length > 0) {
        console.log(
          "Update - Processing footer image file:",
          req.files.footerImage[0].path
        );
        const result = await cloudinary.uploader.upload(
          req.files.footerImage[0].path,
        ...
        footerImage = result.secure_url;
        console.log(
          "Update - Footer image uploaded successfully:",
          footerImage
        );
      }
    }
  ...
  try {
    ...
    try {
      ...
      if (req.body.socialIcons) {
        parsedSocialIcons =
          typeof req.body.socialIcons === "string"
      ...
      if (req.body.recentPosts) {
        parsedRecentPosts =
          typeof req.body.recentPosts === "string"
      ...
      if (req.body.dayTimes) {
        parsedDayTimes =
          typeof req.body.dayTimes === "string"
    ...
    console.log(
      "Checking if logoUrl exists in request body:",
      req.body.logoUrl
    );
    ...
    if (existingFooter) {
      ...
      console.log(
        "Footer already exists, updating instead of creating new one"
      );
...
export const updateFooterTest = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (receivedLogoUrl) {
      console.log(
        "Using logoUrl from request body in update:",
        receivedLogoUrl
      );
