...
export const getAllSpecialOffers = catchAsyncErrors(async (req, res, next) => {
  try {
    const specialOffers = await specialOfferModel.find().sort({ createdAt: -1 });
...
export const createSpecialOfferTest = catchAsyncErrors(async (req, res, next) => {
  try {
    // Process image upload
    let imageUrl = '';
    if (req.file) {
...
export const uploadSpecialOfferImage = catchAsyncErrors(async (req, res, next) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
